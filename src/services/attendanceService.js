...
const checkInService = async (staffId, location) => {
    try {
        ...
        const shiftAssignments = await shiftAssignmentDAO.getShiftAssignmentsByStaff(
            staffId
        );
        ...
        if (todayShift && todayShift.shiftType) {
            ...
            // Tính số phút trễ
            const minutesDiff = calculateMinutesDifference(shiftStartTime, checkInTime);
...
const checkOutService = async (staffId, location) => {
    try {
        ...
        if (todayAttendance.shiftAssignmentId?.shiftType) {
            ...
            // Tính số phút về sớm
            const minutesDiff = calculateMinutesDifference(checkOutTime, shiftEndTime);
...
const getAllAttendancesService = async (startDate, endDate) => {
    try {
        const attendances = await attendanceDAO.getAllAttendances(startDate, endDate);
        return { EC: 0, EM: "Success", data: attendances };
    } catch (error) {
