...
const createShiftAssignmentService = async (assignmentData, createdBy) => {
  try {
    ...
    const departmentShiftExists =
      await departmentShiftDAO.checkDepartmentShiftExists(
        department,
        shiftType,
      );
    ...
    const conflictingAssignment =
      await shiftAssignmentDAO.checkStaffAvailability(staff, fromDate, toDate);
...
const getAllShiftAssignmentsService = async (departmentId, staffId) => {
  try {
    ...
    if (departmentId) {
      assignments =
        await shiftAssignmentDAO.getShiftAssignmentsByDepartment(departmentId);
    } else if (staffId) {
      assignments =
...
const updateShiftAssignmentService = async (id, updateData) => {
  try {
    ...
    if (updateData.fromDate || updateData.toDate) {
      ...
      const conflictingAssignment =
        await shiftAssignmentDAO.checkStaffAvailability(
